...
const draftKey = "rental-minimum-standards-checklist-vic-static:draft";
...
function cacheNodes() {
  ...
  nodes.savedMessage = document.getElementById("saved-message");
  ...
  nodes.saveTop = document.getElementById("save-draft-top");
  nodes.saveBottom = document.getElementById("save-draft-bottom");
}
...
function attachStaticListeners() {
  ...
  nodes.saveTop.addEventListener("click", saveLocalDraft);
  nodes.saveBottom.addEventListener("click", saveLocalDraft);
}
...
function renderChecklistStep(section) {
      ...
      <div class="panel">
        ...
        <label class="field" style="margin-top: 16px;">
          ...
          <div class="priority-grid" role="group" aria-label="Priority">
            ${section.priorityOptions
              .map((priority) => {
                const selected = response.priority === priority ? "selected" : "";
                return `<button class="choice priority-${priority.toLowerCase()} ${selected}" data-priority="${priority}" type="button">${priority}</button>`;
              })
              .join("")}
          </div>
        </label>
  ...
  nodes.screen.querySelectorAll("[data-priority]").forEach((button) => {
    button.addEventListener("click", () => {
      response.priority = button.dataset.priority;
      render();
    });
  });
  ...
  nodes.screen.innerHTML = `
    <section class="screen-stack">
      ...
      <div class="metric-grid">
        ...
        ${renderMetric("Urgent", summary.urgentCount)}
...
function mergeResponses(savedResponses) {
  const initial = createInitialResponses(app.checklist);
  if (!savedResponses) return initial;

  return Object.fromEntries(
    app.checklist.map((section) => {
      const saved = savedResponses[section.id] || {};
      return [
        section.id,
        {
          ...initial[section.id],
...
function saveLocalDraft() {
  ...
  nodes.savedMessage.textContent = `Draft saved on this device at ${new Date(savedAt).toLocaleTimeString("en-AU")}`;
}
...
function restoreDraft() {
  ...
  try {
    ...
    app.property = { ...createDefaultProperty(), ...(draft.property || {}) };
    app.responses = mergeResponses(draft.responses);
    nodes.savedMessage.textContent = `Draft restored from ${new Date(draft.savedAt).toLocaleString("en-AU")}`;
  } catch {
...
function resetDraft() {
  ...
  app.pdfUrl = "";
  nodes.savedMessage.textContent = "Draft cleared";
  render();
  window.scrollTo({ top: 0, behavior: "smooth" });
}
...
function calculateSummary() {
  ...
  const urgentCount = app.checklist.filter((section) => {
    const response = app.responses[section.id];
    return (
      response.priority === "Urgent" &&
      response.assessment !== "pass" &&
  ...
  const highRiskCount = app.checklist.filter((section) => {
    const response = app.responses[section.id];
    return (
      response.priority === "High" &&
      (response.assessment === "fail" || response.assessment === "professional")
    );
  }).length;
  ...
  if (urgentCount > 0 || failedCount > 3 || highRiskCount > 0) {
